...
final statefulList = <ContainerInfo>[
  ...
  ContainerInfo(
      'AlertDialog', '一个通用对话框结构，指定头，中，为出的组件拥有标题，内容的文字样式和编剧，影深，形状等属性', '/alertDialog'),
  ContainerInfo(
      'SpacerWidefe', 'Spacer的通过Expanded的实现的，和Expanded的区别是：Expanded可以设置子控件，而Spacer的子控件尺寸是0，因此Spacer适用于撑开Row、Column、Flex的子控件的空隙', '/SpacerWidefe'),
  ContainerInfo(
      'aboutList', '刷新，控件并没有更新，AboutListTile控件是有点击属性的，点击弹出AboutDialog控件，这些属性出现在AboutDialog控件上，关于AboutDialog的详细内容请查看AboutDialog控件', '/aboutList'),
  ContainerInfo(
      'grid', 'GridTile适合做带有页眉页脚的页面样式，但有一点child 顶部和底部会被页眉页脚覆盖，', '/grid'),
  ContainerInfo(
      'radioLIst', '通常情况下，需要在Radio控件的后面添加说明，用户需要知道自己选择的是什么，当然我们可以直接在Radio后面添加Text控件', '/radioLIst'),
  ContainerInfo(
      'switchList', 'Switch为material风格的开关组件', '/switchList'),
  ContainerInfo(
      'checkboxList', 'Checkbox是勾选框控件，本身不包含任何状态，改变状态需要通过改变value的值改变', '/checkboxList'),
  ContainerInfo(
      'listTitle', 'ListTile是遵循Material Design 规范且固定高度的组件，让开发者快速的构建精美的布局，通常用于ListView的子控件，当然也可以单独使用', '/listTitle'),
];
