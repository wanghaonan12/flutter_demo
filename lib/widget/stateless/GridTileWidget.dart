import 'package:flutter/material.dart';
import 'package:flutter_demo/common/style.dart';

class GridTitleWidget extends StatefulWidget {
  const GridTitleWidget({Key? key}) : super(key: key);

  @override
  State<GridTitleWidget> createState() => _GridTitleWidgetState();
}

class _GridTitleWidgetState extends State<GridTitleWidget> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('GridTitle'),

      ),
      body: Container(
        padding: EdgeInsets.all(10),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [

            const Text(
              '网格列表组件',
              style: titleStyle,
            ),
            Container(
              margin: const EdgeInsets.symmetric(vertical:
              10.0),
              child: const Text(
                'Flutter停工一个通用的列表条目结构',
                style: descStyle,
              ),
            ),
            Container(
              alignment: Alignment.center,
              height: 260,
                child: GridTile(
                  header: GridTileBar(
backgroundColor: Colors.blue.withAlpha(120),
                    trailing: const Icon(
                      Icons.star,color:Colors.red,
                    ),
                    leading: const CircleAvatar
                      (
                      backgroundImage: AssetImage("images/avatar.png"),

                    ),
                    title: const Text('Flutter'),
                    subtitle: const Text('组件学习'),

                  ),
                  footer: const Padding(
                    padding: EdgeInsets.all(8),
                    child: Text(
                      'ID:123456',
                      style: titleStyle,
                    ),
                  ),
                  child: Opacity(
                    opacity: 0.5,
                    child: Image.asset("images/avatar.png",fit: BoxFit.cover,),
                  ),
                ),
            )
          ],
        ),
      ),
    );
  }
}
